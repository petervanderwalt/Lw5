(function () {
    ...
    function millWavyGcode(imageData, opts) {
        ...
        for (var y = yStart; y !== yEnd; y += yStep) {
            var scanLeft = (Math.floor(Math.abs(y - yStart)) % 2 === 0);
            ...
            for (var x = xStart; x !== xEnd; x += xStep) {
                ...
                if (first) {
                    ...
                    lines.push('G1 X' + wx.toFixed(decimal) + ' Y' + wy.toFixed(decimal) + ' Z' + depth.toFixed(decimal) + ' F' + cutFeed);
                }
            }
            lines.push('G0 Z' + safeZ.toFixed(decimal));
        }
